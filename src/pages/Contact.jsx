import {
  PhoneIcon,
  MapPinIcon,
  EnvelopeIcon,
} from "@heroicons/react/24/outline";

export default function Contact() {
  return (
    <>
      <div className="bg-base-200 m-10 py-10 px-5 md:px-10 rounded-2xl shadow-inner">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-4xl font-bold font-aileron text-neutral text-center">Contactez-nous</h1>
          <p className="text-md text-center text-neutral font-lovelace mt-2 mb-10">
            Vous avez <strong>une question</strong> ? Une proposition de <strong>partenariat</strong> ? 
            <br />
            N&apos;hésitez pas à nous contacter par email, nous vous répondrons le plus vite possible.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-center">
            {/* Phone Card */}
            <div className="flex flex-col items-center p-5 shadow-lg rounded-lg bg-base-100">
              <PhoneIcon className="h-10 w-10 text-neutral" />
              <h2 className="font-bold font-aileron text-neutral text-lg mt-2">
                T&eacute;l&eacute;phone
              </h2>
              <p className="text-primary mt-2">
                <a href="tel:+33 3 54 33 27 02" className="link link-hover">
                  (+33) 3 54 33 27 02
                </a>
              </p>
            </div>

            {/* Email Card */}
            <div className="flex flex-col items-center p-5 shadow-lg rounded-lg bg-base-100">
              <EnvelopeIcon className="h-10 w-10 text-neutral" />
              <h2 className="font-bold font-aileron text-neutral text-lg mt-2">
                Email
              </h2>
              <p className="text-primary mt-2">
                <a
                  href="mailto:contact@culinaire.fr"
                  className="link link-hover"
                >
                  contact@culinaire-explore.fr
                </a>{" "}
              </p>
            </div>

            {/* Address Card */}
            <div className="flex flex-col items-center p-5 shadow-lg rounded-lg bg-base-100">
              <MapPinIcon className="h-10 w-10 text-neutral" />
              <h2 className="font-bold font-aileron text-neutral text-lg mt-2">
                Adresse
              </h2>
              <p className="text-primary mt-1">
                8 Rue des Cinq-Piquets, Nancy, 54000, France
              </p>
            </div>
          </div>

          <div className="text-center text-sm text-neutral font-poppins mt-10">
            <p>
              Vous avez un <strong>projet de voyage particulier</strong> ? 
              Une <strong>demande spécifique</strong> ?
              <br/> 
              Vous pouvez nous écrire par email, prendre RDV pour que l&apos;on
              vous appelle (téléphone) ou bien formuler une demande de devis
              ci-dessous :
            </p>
          </div>
        </div>
      </div>
    </>
  );
}
